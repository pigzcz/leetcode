...
public class Question714 {
    ...
    private static int[][]maxr = null;


    private int maxProfit(int[] prices, int fee){
        fn = new int[5000][5000];
        maxr = new int[5000][5000];
        return max(prices,0,prices.length-1,fee);
    }

    private static int max(int[] prices,int k,int n,int fee){
        if (maxr[k][n] !=0){
            return maxr[k][n];
        }
        int max=0;
        ...
        for (int i = k;i<n;i++){
            int m1 = max(prices,k,i,fee) + max(prices,i+1,n,fee);
            int m2 = fn(prices,k,n,fee);
            if (m1>max){
                max = m1;
            }
            if (m2>max){
    ...
    private static int fn(int[] prices, int start, int end, int fee){
        if (fn[start][end] !=0){
            return fn[start][end];
        }
        int maxDifference = 0;
        ...
        for (int i = start;i<=end;i++){
            ...
            if (prices[i]>currentMin){
                if (prices[i] - currentMin -fee > maxDifference){
                    maxDifference = prices[i] - currentMin-fee;
                }
            }
        }
        fn[start][end] = maxDifference;
